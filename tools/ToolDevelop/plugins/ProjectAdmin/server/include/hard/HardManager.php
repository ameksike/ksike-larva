	...
	class HardManager
	{
		public static function makeDir($path, $files)
		{
			...
			switch(gettype($files)){
				case "array" : 
					foreach($files as $i)
						if(!file_exists($path.$i)) mkdir($path.$i, 0777, true);
				break;
				default:
						if(!file_exists($path.$files)) mkdir($path.$files, 0777, true);
				break;
			}
		}
		...
		public static function createFileFromTpl($fileTpl, $fileOut, $values)
		{
			...
			if(file_exists($fileTpl))
			{
				...
				ob_end_clean();
			}else die("Error: La plantilla << {$fileTpl} >> No existe");
		}
		...
		private static function formatTplPHP($data, $in=false)
		{
			if($in){
				...
				$data = str_replace('$', '@', $data);  //... php problem by var
				$data = str_replace('#', '{$', $data); //... php problem by var
			}else   $data = str_replace("@", "$", $data);  //... php problem by var
			return $data;
		}
	}
?>
